...
class CoordTransformer {
  ...
  GCJ02ToBD09(lng, lat) {
    ...
    let z =
      ...
      0.00002 * Math.sin(lat * this.BD_FACTOR);
    let theta =
      Math.atan2(lat, lng) + 0.000003 * Math.cos(lng * this.BD_FACTOR);
  ...
  delta(lng, lat) {
    ...
    dLng =
      (dLng * 180) / ((this.RADIUS / sqrtMagic) * Math.cos(radLat) * this.PI);
    dLat =
      (dLat * 180) /
      (((this.RADIUS * (1 - this.EE)) / (magic * sqrtMagic)) * this.PI);
    return [dLng, dLat];
  }
  ...
  static transformCartesian3ToCartographic(cartesianPosition) {
    if (!Array.isArray(cartesianPosition))
      ...
      let result = [];
      cartesianPosition.forEach((cartesian) =>
        result.push(this.transformCartesianToWGS84(cartesian))
      );
      return result;
    }
  }
  ...
  static transformCartographicToCartesian3(wgs84Position) {
    if (!Array.isArray(wgs84Position))
      ...
      let result = [];
      wgs84Position.forEach((wgs84) =>
        result.push(this.transformWGS84ToCartesian(wgs84))
      );
      return result;
    }
  }
