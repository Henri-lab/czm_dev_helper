...
export function collapse(model, reset, customAnimate) {
    once && (originalPositions = model.modelMatrix.clone());//once call
    once = 0
    if (reset) {
        // console.log('collapse reset')
        model.modelMatrix = originalPositions
        return
    }
    ...
    if (!customAnimate) {
        ...
        animate = (t) => {
            ...
            model.modelMatrix = Cesium.Matrix4.clone(Cesium.Matrix4.IDENTITY);

            // 应用新的变换
            Cesium.Matrix4.multiply(model.modelMatrix, transformMatrix, model.modelMatrix);
...
//     // 应用新的变换到新矩阵
//     Cesium.Matrix4.multiply(model.modelMatrix, transformMatrix, newModelMatrix);
