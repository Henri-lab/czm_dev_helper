...
class DataPrepocesser {
  ...
  static defaultImageUrl(defaultImageUrl: any) {
   ...
   */
  // 修改3dtiles位置
  static update3DtilesMaxtrix(
    tileSet,
    { tx = 0, ty = 0, tz = 0, rx = 0, ry = 0, rz = 0, scale = 1.0 }
  ) {
    const cartoDegree = this.getCenterDegreeFrom3dTiles(tileSet);

    const surface = Cesium.Cartesian3.fromDegrees(
      cartoDegree.longitude,
      cartoDegree.latitude,
      cartoDegree.height
    );
  ...
  static getCenterDegreeFrom3dTiles(tileset) {
    ...
    // 如果是 OrientedBoundingBox
      ...
      boundingVolume.boundingVolume instanceof Cesium.OrientedBoundingBox
    ) {
      const center = boundingVolume.boundingVolume.center;
      const cartographic = Cesium.Cartographic.fromCartesian(center);
      const latitude = Cesium.Math.toDegrees(cartographic.latitude);
  ...
  static getCenterOfPrimitives(
    ...
    if (primitives.every((item) => item instanceof Cesium.PointPrimitive)) {
      ...
      let boundingSphere = Cesium.BoundingSphere.fromPoints([
        primitives[0].position,
      ]);

      // 遍历其余的 primitives 并扩展 boundingSphere
      for (let i = 1; i < primitives.length; i++) {
        const primitive = primitives[i];
        Cesium.BoundingSphere.expand(
          boundingSphere,
          primitive.position,
          Cesium.BoundingSphere.fromPoints([primitive.position])
        );
      }
