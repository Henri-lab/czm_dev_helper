...
const createDynamicPoint = (_viewer_, colorsProp, positionProp) => {
    if (!_viewer_ || !colorsProp) return
...
onMounted(() => {
    ...
    timer2 = setTimeout(() => createDynamicPoint(_viewer_, props.colors, props.position), 10)
})
watch(() => props.colors, (newV) => {
    createDynamicPoint(_viewer_, newV, props.position)
})
watch(() => props.position, (newV) => {
    createDynamicPoint(_viewer_, props.colors, newV)
})
